...
window.onload = () => {
  ...
  canvas.addEventListener('touchend', e => {
    ...
    let dxT=t.clientX-startX;
    let dyT=t.clientY-startY;

    if(Math.abs(dxT)>Math.abs(dyT)){
      if(dxT>0 && dx===0){dx=size; dy=0;}
      else if(dxT<0 && dx===0){dx=-size; dy=0;}
    }else{
      if(dyT>0 && dy===0){dx=0; dy=size;}
      else if(dyT<0 && dy===0){dx=0; dy=-size;}
    }
  });
};

function startGame(){
  worm=[{x:300,y:300},{x:280,y:300},{x:260,y:300}];
  dx=size; dy=0;
  cupcakes=[];
...
function spawnCupcake(){
  cupcakes.push({x:Math.floor(Math.random()*(width/size))*size,
...
function loop(){
 if(!running)return;
 setTimeout(()=>{move();draw();loop();},100);
}

function move(){
 let head={x:worm[0].x+dx,y:worm[0].y+dy};
 if(head.x<0||head.x>=width||head.y<0||head.y>=height){
  running=false; alert('Spiel vorbei'); return;
 }
 worm.unshift(head);

 let ate=false;
 cupcakes=cupcakes.filter(c=>{
  if(Math.abs(head.x-c.x)<size && Math.abs(head.y-c.y)<size){
    ate=true; return false;
  }
  return true;
 });
 if(ate){spawnCupcake(); if(worm.length>120)worm.pop();}
 else worm.pop();
}

function draw(){
 ctx.clearRect(0,0,width,height);
 ctx.strokeStyle='#2ecc71'; ctx.lineWidth=size;
 ctx.beginPath();
